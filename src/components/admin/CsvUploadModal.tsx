...
export function CsvUploadModal({ quiz, isOpen, onClose, onImportQuestions }: CsvUploadModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto space-y-6 shadow-2xl text-slate-100">
        ...
        <div className="flex items-start justify-between pb-4 border-b border-slate-800">
          <div>
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Import Questions via CSV File</h3>
        ...
        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-blue-300 hover:text-blue-200 border border-slate-700 px-3.5 py-2 rounded-xl text-xs font-bold transition-colors shrink-0"
          >
        ...
        {activeTab === 'paste' && (
          <div className="space-y-2">
            <textarea
              ...
              className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-xs font-mono text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 leading-relaxed"
            />
            <p className="text-[10px] text-slate-500">
            ...
            {parsedResult.validQuestions.length > 0 && (
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {parsedResult.validQuestions.map((q, idx) => (
                  <div 
                    key={q.id || idx}
                    className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-start justify-between gap-3 text-xs"
                  >
                    <div className="space-y-1.5 flex-1">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          {q.hindiQuestion && (
                            <p className="text-[11px] text-slate-400">{q.hindiQuestion}</p>
                          )}
                        </div>
                      </div>
                    ...
                    <button
                      ...
                      className="p-1 text-slate-500 hover:text-rose-400 hover:bg-slate-900 rounded transition-colors shrink-0"
